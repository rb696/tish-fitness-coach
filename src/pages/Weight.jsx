...
export default function Weight() {
  ...
  if (weekLogs.length >= 2) {
    const wFirst = weekLogs[0], wLast = weekLogs[weekLogs.length - 1]
    ...
    weeklyTrend = {
      range: `${fD(wFirst.logged_date)} – ${fD(wLast.logged_date)}`,
  ...
  const subtitles = {
    weight:  'Log weekly · track your recomp progress',
    steps:   'Goal: 8,000 steps per day',
    history: 'Your past weigh-ins and step logs',
  }
  ...
  return (
    <div className="pb-28 max-w-lg mx-auto">
      ...
      <div className="px-4 pt-6 mb-4">
        ...
        <p className="text-gray-400 text-sm mt-1">{subtitles[activeTab]}</p>
      </div>

      {/* Tab bar — exact Gym pattern */}
      ...
      {activeTab === 'history' && (
        <div className="px-4 space-y-6">
          ...
          {/* Weigh-ins history */}
          <div>
            <p className="text-white font-semibold text-sm mb-3">Weigh-ins</p>
            {logs.length === 0 ? (
              ...
              <div className="bg-[#1e1e2a] rounded-2xl border border-white/5 overflow-hidden">
                {[...logs].reverse().slice(0, 30).map((log, i, arr) => {
          ...
          {/* Steps history */}
          <div>
            <p className="text-white font-semibold text-sm mb-3">Steps</p>
            {stepLogs.length === 0 ? (
              <div className="bg-[#1e1e2a] rounded-2xl border border-white/5 p-6 text-center">
              ...
              <div className="bg-[#1e1e2a] rounded-2xl border border-white/5 overflow-hidden">
                {[...stepLogs].reverse().slice(0, 30).map((log, i, arr) => {
                  ...
                  return (
                    <div key={log.id} className={`px-4 pt-3 pb-2.5 ${!isLast ? 'border-b border-white/5' : ''}`}>
                      ...
                      <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${hitGoal ? 'bg-emerald-500' : 'bg-amber-500/60'}`} style={{ width: `${pct}%` }} />
                      </div>
                      {!hitGoal && <p className="text-gray-600 text-[10px] mt-1">{(STEP_GOAL - log.steps).toLocaleString()} to go</p>}
                    </div>
                  )
                })}
              </div>
